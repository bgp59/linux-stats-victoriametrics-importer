...
type CompressorPoolTestCase struct {
	...
	// be supplied with the expected type for the fields (e.g. FlushInterval
	// string):
	NumCompressors   any
	CompressLevel    any
	BatchTargetSize  any
...
func makeTestCompressorPool(tc *CompressorPoolTestCase) (*CompressorPool, error) {
	...
	if flushInterval, ok := tc.FlushInterval.(string); ok {
...
func TestCompressorPoolCreate(t *testing.T) {
	for _, tc := range []*CompressorPoolTestCase{
		...
		{
			FlushInterval: "13",
			wantError:     fmt.Errorf(`NewCompressorPool: invalid flush_interval "13": time: missing unit in duration "13"`),
		},
	} {
		t.Run(
			"",
