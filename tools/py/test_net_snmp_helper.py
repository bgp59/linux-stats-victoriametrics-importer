...
from testutils import procfs_proc_root_dir, procfs_testdata_root

reference_net_snmp_file = os.path.join(procfs_proc_root_dir, "net", "snmp")
output_net_snmp_file = os.path.join(
    procfs_testdata_root, "net", "snmp", "field_mapping", "net", "snmp"
)

net_snmp_signed_value_names = set(["TcpMaxConn"])
...
if __name__ == "__main__":
    ...
    with open(reference_net_snmp_file) as f:
        ...
        for line in f:
            ...
            if line_num % 2 == 1:
                ...
                proto = prefix[:-1]
                names = [proto + stat for stat in stats]
                num_vals = len(names)
                values = [1000 * line_num + k for k in range(num_vals)]
    ...
    # primeNetSnmp start:
    print("\t" * indent_lvl + "primeNetSnmp: &NetSnmp{")
    indent_lvl += 1
    ...
    for names in names_by_line:
        print("\t" * indent_lvl + ", ".join(map(lambda s: f'"{s}"', names)) + ",")
    indent_lvl -= 1
    print("\t" * indent_lvl + "},")
    ...
    print(
        "\t" * indent_lvl
        + "Values: make([]int64, "
        + "+".join(map(lambda v: str(len(v)), values_by_line))
        + "),"
    )

    # primeNetSnmp - lineInfo:
    print("\t" * indent_lvl + "lineInfo: []*NetSnmpLineInfo{")
    indent_lvl += 1
    ...
    # wantNetSnmp start:
    print("\t" * indent_lvl + "wantNetSnmp: &NetSnmp{")
    ...
    # wantNetSnmp - Values:
    print("\t" * indent_lvl + "Values: []int64{")
    indent_lvl += 1
    for values in values_by_line:
        print("\t" * indent_lvl + ", ".join(map(str, values)) + ",")
    indent_lvl -= 1
    print("\t" * indent_lvl + "}")
