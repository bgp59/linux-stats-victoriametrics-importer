...
if __name__ == "__main__":
    ...
    uid_list = [
        ...
        if (
            ...
            and (folder is None or d.get("folderTitle", "") == folder)
        )
    ]
    ...
    for uid in uid_list:
        ...
        norm_title, norm_folder = normalize_title(dashboard["title"]), normalize_title(
            meta["folderTitle"]
        )
        if out_dir is not None:
        ...
        if args.normalize_uid:
            org_uid, new_uid = (
                dashboard["uid"],
                norm_title,
            )  # f"{norm_folder}_{norm_title}"
            if org_uid != new_uid:
                dashboard["uid"] = new_uid
                print(
                    f'dashboard: {dashboard["title"]!r} folder: {meta["folderTitle"]!r}: uid: {org_uid!r} -> {new_uid!r}',
                    file=sys.stderr,
                )
