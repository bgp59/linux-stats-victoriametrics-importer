...
def generate_compressor_metrics(
    ...
    for i, metric_name in compressor_stats_uint64_delta_metric_names.items():
        val = curr_compressor_stats[UINT64_STATS_FIELD][i]
    ...
    for i, metric_name in compressor_stats_float64_metric_names.items():
        val = curr_compressor_stats[FLOAT64_STATS_FIELD][i]
        metrics.append(
            f"{metric_name}{{"
            + ",".join(
                [
...
def generate_compressor_pool_internal_metrics_test_cases(
    ...
    for compressor_id in curr_stats:
        k += 1
        for i in range(len(curr_stats[compressor_id][UINT64_STATS_FIELD])):
            curr_stats[compressor_id][UINT64_STATS_FIELD][i] += 100 * k + i
        for i in range(len(curr_stats[compressor_id][FLOAT64_STATS_FIELD])):
            curr_stats[compressor_id][FLOAT64_STATS_FIELD][i] += 1000 * k + i
    test_cases.append(
        generate_compressor_pool_internal_metrics_test_case(
            f"{tc_num:04d}",
            curr_stats,
